...
func (db *DatabaseAdapter) FindLastVaultBlockByChain(sourceChain string) ([]*chains.VaultTransaction, error) {
	...
	query := `
		...
		WHERE vt.chain = $1 AND vt.block_number = (
			SELECT max(vt2.block_number) FROM vault_transactions vt2 join command_executeds ce 
			on vt2.tx_hash = ce.command_id and 'evm|'||vt2.destination_chain = ce.source_chain
			WHERE vt2.chain = $1 AND vt2.vault_tx_type = 1
		)
	`
